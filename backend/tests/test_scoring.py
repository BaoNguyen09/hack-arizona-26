...
def test_zero_weights_give_zero_score() -> None:
    score = compute_composite_score(
        np.array([60.0]), np.array([34.0]), np.array([20.0]),
        0.0, 0.0, 0.0,
    )
    assert score[0] == 0.0
...
def test_score_all_cells_returns_required_columns() -> None:
    ...
    for col in ("selected_cf", "lcoe_usd_per_mwh", "revenue_usd_per_mwh",
                "carbon_value_usd_per_mwh", "composite_score", "annual_generation_mwh"):
...
def test_score_all_cells_wind_uses_wind_cf() -> None:
    wind_scenario = ScenarioRequest(**{**DEFAULT_SCENARIO.model_dump(), "technology": "wind"})
...
def test_score_single_cell_keys_present() -> None:
    ...
    for key in (
        "lcoe_usd_per_mwh", "lcoe_components", "revenue_usd_per_mwh",
        "carbon_value_usd_per_mwh", "annual_generation_mwh", "annual_generation_gwh",
        "capex_total_usd_millions", "annual_revenue_usd_millions",
        "annual_carbon_displacement_tons", "annual_carbon_value_usd", "selected_cf",
    ):
...
def test_score_single_cell_annual_carbon_tons() -> None:
    ...
    assert pytest.approx(metrics["annual_carbon_displacement_tons"], rel=1e-4) == expected_tons
